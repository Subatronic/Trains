...
function normalizeResolvedLocation(location) {
  ...
  return location.map((item) => item?.name || item?.signature).filter(Boolean).join(", ");
}

function getDirection(train) {
  if (els.activityType.value === "Avgang") {
    return normalizeResolvedLocation(train.ToLocationResolved) || normalizeLocation(train.ToLocation);
  }
  return normalizeResolvedLocation(train.FromLocationResolved) || normalizeLocation(train.FromLocation);
}
